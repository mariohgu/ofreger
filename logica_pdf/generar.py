...
pdf_path = os.path.join(file, 'documentos', '03.pdf')

text = extract_text(pdf_path)

# Extraer texto usando las expresiones regulares definidas
description_text = extract_using_regex(r'del peligro inminente:\s+(.*?)\s+\d+\.\d+\.+\s+[A-Z]', text)
tumbes_text = extract_using_regex(r'Ubigeo:\s+\nTumbes\s+(.*?)\s+\d+\.\d+\.+\s+[A-Z]', text)
